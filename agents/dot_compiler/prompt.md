...
- One input port node per input port (left side)
- One core node (center) with shape=box3d
- One output port node per output port (right side)
- Internal dashed edges from input ports → core → output ports
...
- Every input port has a `<module_type>_in_<port_key>` node with class="input port".
- Every output port has a `<module_type>_out_<port_key>` node with class="output port".
- Internal dashed edges connect input ports → core → output ports inside each cluster.
- Inter-module edges connect `_out_` nodes to `_in_` nodes (NOT core-to-core).
- Global signals (3+ fanout) use the relay pattern with taps connecting to `_in_<signal>` nodes.
