...
class Port(BaseModel):
    ...
    direction: str = Field(description="Specifies if signal flows into (input) or out of (output) the module")
    width: str = Field(description="Bit-width of the signal, essential for labeling bus vs. single wire")
...
class LogicBlock(BaseModel):
    ...
    module_type: str = Field(description="The name of the module being used (e.g., 'ctrl')")
    port_mapping: Dict[str, str] = Field(
        description="A simple flat dictionary where the KEY is the module port name and the VALUE is the connected wire name. Example: {'clk': 'clk', 'start': 'w_start'}"
)
...
class InternalWire(BaseModel):
    ...
    width: str = Field(description="Bit-width of the wire, helps in visual weight of diagram lines")

class RTLStructure(BaseModel):
    """
    The root container for the entire parsed RTL design.
    This class is the 'Source of Truth' for the Agentic System; all downstream 
    styling and DOT generation agents rely on the data structured here.
    """
    ...
    internal_wires: List[InternalWire] = Field(description="List of all signals defined for inter-module connectivity")
    instances: List[LogicBlock] = Field(description="List of all sub-components found inside the design")